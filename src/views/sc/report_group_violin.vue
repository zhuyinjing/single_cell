    ...
    <el-tabs v-model="activeTab" v-show="violinSvgShow || heatmapSvgShow || scatterSvgShow">
      ...
      <el-tab-pane style="overflow-x:auto" label="FeaturePlot" name="scatterSvgShow">
        <div class="scatter" style="white-space: nowrap;">
          ...
          &nbsp;&nbsp;&nbsp;
          {{$t('d3.height')}}：<el-input-number size="mini" v-model="heightFeature" :step="100" :min="0" @change="changeWidth()"></el-input-number>
          &nbsp;&nbsp;&nbsp;
          是否使用相同的基因表达量最大值最小值：
          <el-switch
            v-model="colorSwitch"
  ...
  methods: {
    ...
    initScatter () {
      ...
      for (let i = 0;i < this.selected.length;i++) {
        ...
        // let colorScale = d3.scaleSequential(d3Chromatic.interpolateReds).domain(d3.extent(colorDomainArr))
         ...
         var defs = svg.append("defs");
          ...
          linearGradient.append("stop")
       		...
       		var minValueText = svg.append("text")
       					...
       					.attr("dy", "-0.3em")
                .attr("text-anchor", "middle")
                // .text(() => d3.min(colorDomainArr).toFixed(2));
